...
public class Checkpoint : MonoBehaviour
{
	...
	private RacingFeatureLayerComponent racingComponent;
	private SplineContainer splineContainer;
	private ArcGISMapComponent mapComponent;

	public void Initialize (RacingFeatureLayerComponent racingComponent, SplineContainer splineContainer, ArcGISMapComponent mapComponent, ArcGISPoint point)
	{
		this.racingComponent = racingComponent;
	...
	private void UpdateSplinePosition (ArcGISPoint point)
	{
		...
		bezierKnot.Position = mapComponent.GeographicToEngine(point);
		transform.SetPositionAndRotation(bezierKnot.Position, racingComponent.GetSplineKnotRotation(SplineIndex + 1, SplineIndex));
		splineContainer.Splines[0][SplineIndex] = bezierKnot;

		racingComponent.ResetRaceLoadingTimer( );
	}
}
